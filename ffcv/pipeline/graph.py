...
# 管道初始状态配置
INITIAL_STATE = State(jit_mode=True,    # 默认启用JIT编译
                       device=ch.device('cpu'), # 默认设备为CPU
                       dtype=np.dtype('u1'),    # 默认数据类型为无符号8位整数
                       shape=None)  # 默认形状为None

# 管道节点抽象基类（表示数据处理流程中的一个步骤）
class Node(ABC):
    last_node_id: int = 0   # 用于生成唯一节点ID的计数器
    def __init__(self):
        self.id = Node.last_node_id # 生成唯一ID
        self._code = None   # 保存编译后的代码
        Node.last_node_id += 1

    @property
    @abstractmethod
    def is_jitted(self):    # 是否启用JIT编译
        raise NotImplemented()

    @property
    @abstractmethod
    def parent(self):   # 获取父节点（数据来源）
        raise NotImplemented()
    
    @property
    @abstractmethod
    def arg_id(self):   # 获取输入参数标识符
        raise NotImplemented()
    
    @property
    @abstractmethod
    def result_id(self):    # 获取输出结果标识符
        raise NotImplemented()
    ...
    def get_shared_code_ast(self, done_ops):     # 生成共享状态代码的AST（抽象语法树）
        return ast.Pass()
    
    @abstractmethod
    def generate_code(self):    # 生成节点的处理代码
        raise NotImplemented()

    def recompile(self):    # 重新编译节点代码
        self._code = self.generate_code()

    @property
    def with_indices(self): # 是否需要批次索引
        try:
    ...
    @property   
    def func_call_ast(self):    # 生成函数调用的AST
...
class DecoderNode(Node):
    ...
    @property
    def parent(self):   # 解码器节点没有父节点
        return None

    @property
    def arg_id(self):   # 使用批次索引作为输入
        return 'batch_indices'

    @property
    def result_id(self):    # 输出结果标识符
        return f"result_{self.id}"

    def generate_code(self):    # 生成解码器代码
        return self.decoder.generate_code()

    @property
    def func_call_ast(self):    # 生成包含元数据和存储状态的特殊调用AST
...
class Graph:
    ...
    def __init__(self, pipeline_specs: Dict[str, PipelineSpec], handlers,
        ...
        self.memory_read = memory_read  # 内存读取接口
        self.handlers = handlers    # 字段处理器
        self.fieldname_to_fix = fieldname_to_fix    # 字段名到索引的映射
        self.metadata = metadata    # 元数据
        self.pipeline_specs = pipeline_specs    # 管道规范字典
        self.nodes: List[Node] = [] # 管道节点列表
        self.root_nodes: Dict[Node, str] = {}   # 根节点
        self.leaf_nodes: Dict[str, Node] = {}   # 叶节点
        self.operation_to_node = defaultdict(list)  # 操作到节点的映射
        self.id_to_node = {}    # 节点ID到节点的映射
        self.node_to_id = {}    # 节点到ID的映射
