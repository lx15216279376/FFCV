...
Section('cfg', 'arguments to give the writer').params(
    ...
    write_mode=Param(str, 'Mode: raw, smart or jpg', required=False, default='smart'),  # 编码策略,raw:原始 RGB uint8 像素数组（H×W×3）,不需要解码；jpg: JPEG 编码的图像，解码后为 RGB uint8 像素数组（H×W×3）；smart: smart JPEG 编码的图像，解码后为 RGB uint8 像素数组（H×W×3）
    max_resolution=Param(int, 'Max image side length', required=True),
    num_workers=Param(int, 'Number of workers to use', default=16),
    chunk_size=Param(int, 'Chunk size for writing', default=100),   # 每次写入的图片数量
