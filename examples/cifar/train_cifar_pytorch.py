...
def make_dataloaders(train_dataset=None, val_dataset=None, batch_size=None, num_workers=None):
    ...
    # CIFAR-10 数据集
    # 目录应包含 train_dataset/cifar-10-batches-py/ 和 val_dataset/cifar-10-batches-py/
    train_dataset = datasets.CIFAR10(root=train_dataset, train=True, download=True, transform=train_transform)
    val_dataset = datasets.CIFAR10(root=val_dataset, train=False, download=True, transform=val_transform)

    train_loader = ch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True)
    val_loader = ch.utils.data.DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)

    return {'train': train_loader, 'test': val_loader}, start_time
